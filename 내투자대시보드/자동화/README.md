...
```
[매 거래일 22:00 UTC]  collect.py
   환율(USD/KRW) · 벤치마크(S&P·NASDAQ·다우) · 종목시세 · Fear&Greed
        └─►  데이터/*.csv 에 누적 (같은 날짜는 덮어쓰기)

[매월 1일 01:00 UTC]  build_ledger.py
...
2. 매월 `자동수집_원장.xlsx` 를 열어 **`월별스냅샷`**(자산군별 기말평가액·당월순입금 + MMF·RP·현금 잔고)과 **`목표`** 만 채운다
