...
당신이 채우는 것 (개인 데이터):
  · 월별스냅샷: 자산군별 기말평가액 + 당월순입금 (+ MMF·RP·현금 잔고)
  · 목표      : 목표금액·목표일·월저축액

즉 이 파일을 열어 '월별스냅샷'·'목표'만 채우면 원장이 완성됩니다.
"""
...
def main():
    ...
    g["A2"] = "'월별스냅샷'과 '목표'만 채우면 대시보드에 넣을 수 있습니다."
    g["A2"].font = Font(italic=True, color="666666")
    g.column_dimensions["A"].width = 60

    # 월별스냅샷 — 헤더만 (사용자 입력)
    ws = wb.create_sheet("월별스냅샷")
    header(ws, "월별 스냅샷 — 자산군별로 한 줄씩 (직접 입력)",
           "기준월(YYYY-MM)·자산군·통화·기말평가액·당월순입금. 자산군 6종만.",
